...
try {
   ...
   if (!user) return res.status(404).send({msg : 'المستخدم غير موجود!'});
    ...
    const valid = await bcrypt.compare(req.body.dashboard_password, user.dashboard_password);
   if (!valid) return res.status(400).send({msg : 'البريد الإلكتروني أو كلمة المرور غير صحيحة'});
...
const updateAdmin = async (req, res) => {
  try {
    ...
    const isMatch = await bcrypt.compare(req.body.old_pass, user.dashboard_password);
    ...
    if (!isMatch) {
      ...
      if (req.body.dashboard_password) {
        const salt = await bcrypt.genSalt(10);
        req.body.dashboard_password = await bcrypt.hash(req.body.dashboard_password, salt);
      }

      const updated = await Auth.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true });
